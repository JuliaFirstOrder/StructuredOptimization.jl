...
fft{D1<:Complex}(x::OptVar{D1}) = DFT{D1,D1}(x, plan_fft(x.x), plan_ifft(x.x),(size(x),size(x))) 
...
ifft{D1<:Complex}(x::OptVar{D1}) = IDFT{D1,D1}(x, plan_ifft(x.x), plan_fft(x.x),(size(x),size(x))) 
ifft{D1<:Float64}(x::OptVar{D1}) = 
IDFT{D1,Complex{Float64}}(x, plan_ifft(x.x), plan_fft(complex(x.x)),(size(x),size(x))) 
...
transpose{D1,D2}(A::IDFT{D1,D2})  =  DFT{D2,D1}(A.x, A.Ainv, A.A, A.dim )
